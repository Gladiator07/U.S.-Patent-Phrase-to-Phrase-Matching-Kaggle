    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if drop_parameters is not None:
            ...
            del state_dict
        if pretrained_model_name_or_path in pretrained_model_archive_map:
            pretrained_model_name_or_path = pretrained_model_archive_map[
                pretrained_model_name_or_path
            ]
...
class COCOLMLayer(nn.Module):
        ...
        if self.layer_norm_type in ("pre", "hybrid"):
            if self.num_ffn_layers > 0:
                self.LayerNorm = nn.ModuleList(
                    [
                        COCOLMLayerNorm(config.hidden_size, eps=1e-5)
    ...
    def forward(
        ...
        if isinstance(self.intermediate, nn.ModuleList):
            ...
            for i, (intermediate_layer, output_layer) in enumerate(
                zip(self.intermediate, self.output)
            ):
                if self.layer_norm_type in ("pre", "hybrid"):
...
class COCOLMEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [COCOLMLayer(config) for _ in range(config.num_hidden_layers)]
        )
        self.layer_norm_type = config.layer_norm_type
        if self.layer_norm_type in ("pre", "hybrid"):
            self.LayerNorm = COCOLMLayerNorm(config.hidden_size, eps=1e-5)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if (self.layer_norm_type in ("pre", "hybrid")) and (
                i == len(self.layer) - 1
            ):
...
class COCOLMModel(COCOLMPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.rel_pos_bins > 0:
            ...
            self.relative_attention_bias = nn.Embedding(
                self.config.rel_pos_bins, self.config.num_attention_heads
            )
    ...
    def forward(
        ...
        rel_pos_bias = (
            self.get_rel_pos_bias(input_ids).repeat(input_ids.size(0), 1, 1)
            if self.config.rel_pos_bins > 0
            else None
        )
        ...
        if rel_pos_bias is not None and extended_attention_mask is not None:
            ...
            rel_pos_bias.masked_fill_(
                extended_attention_mask.unsqueeze(1).unsqueeze(2), float("-inf")
            )
